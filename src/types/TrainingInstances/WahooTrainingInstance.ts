...
export class WahooTrainingInstance extends TrainingInstance {
    ...
    constructor(title: string, targetedTrainingTime: number, targetPowerZones: Metric[] = []) {
        ...
        this.registerHandler((trainingSnapshot: TrainingSnapshot) => {
            const currentPowerTarget = this._currentInstantaneousPower * this.instantenousPowerMultiplier;
            this.instanteneousPower.next({ts: trainingSnapshot.currentTrainingDifference, target: currentPowerTarget});
        });
        ...
        this.registerHandler(async (trainingSnapshot: TrainingSnapshot) => {
            ...
            const powerTargetChanged = (trainingSnapshot.lastPowerZone.target * this.instantenousPowerMultiplier) !== this._currentTargetPower;
            if(!powerTargetChanged && !targetPowerZone){
               return; 
            }
            const nextTargetPower = Math.round((targetPowerZone?.target ?? trainingSnapshot.lastPowerZone.target) * this.instantenousPowerMultiplier);
            ...
            this._currentTargetPower = nextTargetPower;
